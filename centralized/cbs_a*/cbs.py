...
class Constraints(object):
    ...
    def add_vertex_cons(self, other):
        self.vertex_constraints |= set([other])

    def add_edge_cons(self, other):
        self.edge_constraints |= set([other])
...
class Environment(object):
    ...
    def compute_solution(self, START_TIME):
        ...
        for agent in self.agent_dict.keys():
            end_time = time.time()
            if end_time - START_TIME >= TIME_LIMITATION:
                # print(end_time)
                # print(START_TIME)
            ...
            # print(len(self.constraints.vertex_constraints))
            local_solution = self.a_star.search(agent, START_TIME)
            if local_solution == 'TIME':
                return 'TIME'


            # add pt in previous path as constraint
            # print('local_solution: ' + str(local_solution))
            for i, state in enumerate(local_solution):
                t = state.time
                location = state.location
                location2 = local_solution[i + 1].location if i < len(local_solution) - 1 else None
                # x = state.location.x
                # y = state.location.y
                v_constraint = VertexConstraint(t, location)
                self.constraints.add_vertex_cons(v_constraint)
                if location2:
                    e_constraint = EdgeConstraint(t, location2, location)
                
                    self.constraints.add_edge_cons(e_constraint)
...
class Multi_Astar(object):
    def __init__(self, environment, result_pth):
        self.env = environment
        self.result_pth = result_pth

    def search(self, START_TIME):
        start_time = time.time()
        solution = self.env.compute_solution(START_TIME)
        if solution == 'TIME':
            return {}
        if not solution:
            return {}
        end_time = time.time()
        cost = self.env.compute_solution_cost(solution)
        f = open(self.result_pth, 'a')
        print(self.result_pth)
        print('Cost: ' + str(cost) + '  |  Time: ' + str(end_time - start_time) + '   \n')
        f.write('Cost: ' + str(cost) + '  |  Time: ' + str(end_time - start_time) + '   \n')
        f.close()
...
def main():
    ...
    # Searching
    START_TIME = time.time()
    cbs = Multi_Astar(env, args.result_pth)
    print('Start searching ...')
    solution = cbs.search(START_TIME)
