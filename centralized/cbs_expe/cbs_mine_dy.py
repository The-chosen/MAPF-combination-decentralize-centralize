...
class Environment(object):
    def __init__(self, dimension, agents, obstacles, obstacles_d=[], w_l=W_L, alpha1=1, alpha2=0.75, alpha3=1):
        print("[INFO] Position affected by dynamic obstacles: " + str(obstacles_d))
        # f = open('./dy_obs.txt', 'a')
        # f.write('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n')
        # f.write(str(obstacles_d) + '\n')
        # f.write('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n')
        # f.close()
    ...
    def state_valid(self, state):
        # if state.location.x == 8 and state.location.y == 1:
            # f = open('./dy_obs.txt', 'a')
            # f.write(str(self.obstacles_d))
            # f.write(str((state.location.x, state.location.y) not in self.obstacles_d) + '\n')
            # f.close()
...
def dy_pos_blur(x, y, seed):
    # return (x, y)
    blur_length = 3
    blur_area = [(x + i, y + j) for i in range(- blur_length, 1 + blur_length) for j in range(- blur_length, 1 + blur_length)]
    random.seed(seed)
...
def replanning():
    ...
    dimension = input_["map"]["dimensions"]
    obstacles = input_["map"]["obstacles"]
    agents = input_['agents']

    goals_points = [agent['goal'] for agent in agents]
    ...
    traj = cbs.search(N, agent_num, obstacle_prob)
    print('[INFO] Initial searching end')
    print('agents: ', agents)
    if not traj:
        print('Initial solution not found!')
        return
    ...
    if 'without' not in args.dynamic_obs:
        max_t_agent = max([len(pth) for pth in traj.values()])
        max_t_dy = max(len(pth) for pth in dy_obs['schedule'])
        max_t = max(max_t_agent, max_t_dy)



        print('[INFO] Found initial solution. ')
        ...
        for t in range(max_t):
            ...
            dy_obs_positions = [] if len(dy_obs_positions) == 1 else dy_obs_positions
            
            
            print('Dy_obs_positions number: ', len(dy_obs_positions))
            ...
            for i, pth in enumerate(solution.values()):
                if t + 1 <= len(pth) - 1:
                    ...
                    new_agents.append({'start': [pth[t + 1]['x'], pth[t + 1]['y']], 'goal': agents[i]['goal'], 'name': agents[i]['name']})
                    # print(new_agents)
                    # print('---------------')
                    # agents = [{'start': [pth[t]['x'], pth[t]['y']], 'goal': agent['goal'], 'name': agent['name']} for agent in agents]
                    new_start.append([pth[t]['x'], pth[t]['y']])
                else:
                    new_agents.append({'start': [pth[-1]['x'], pth[-1]['y']], 'goal': agents[i]['goal'], 'name': agents[i]['name']})
                    # agents = [{'start': [pth[len(pth) - 1]['x'], pth[len(pth) - 1]['y']], 'goal': agent['goal'],\
                    #      'name': agent['name']} for agent in agents]
                    new_start.append([pth[len(pth) - 1]['x'], pth[len(pth) - 1]['y']])

            dy_obs_positions = [pt for pt in dy_obs_positions if [pt[0], pt[1]] not in goals_points and [pt[0], pt[1]] not in new_start]

            N, agent_num, obstacle_prob = None, None, None
            agents = new_agents
            print('obstacles: ' + str(obstacles))
            print('dy_obs_positions: ' + str(dy_obs_positions))
            print("+ :" + str(obstacles + dy_obs_positions))
            print('agents: ', new_agents)
            env = Environment(dimension, new_agents, obstacles + dy_obs_positions)
            cbs = CBS(env, TIMESTEP)
            print('[INFO] Start initial searching ...')
            traj = cbs.search(N, agent_num, obstacle_prob)
            print(traj)
            ...
            if not traj:
                ...
                for i, pth in enumerate(solution.values()):
                    if t <= len(pth) - 1:
                        # print("=======================")
                        # print(solution['agent' + str(i)][:t + 1])
                        # print([{'t': int(str(j + t + 1)), 'x': int(str(pt['x'])), 'y': int(str(pt['y']))} \
                        #         for j, pt in enumerate(traj['agent' + str(i)])])
                        # print("=======================")
                        solution['agent' + str(i)] = solution['agent' + str(i)]\
                            [:t + 1] + [{'t': int(str(j + t + 1)), 'x': int(str(pt['x'])), 'y': int(str(pt['y']))} \
                                for j, pt in enumerate(traj['agent' + str(i)])]
