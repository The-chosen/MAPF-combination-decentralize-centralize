...
class AStar():
    ...
    def search(self, agent_name, w_l, alpha1, alpha2, alpha3, pre_solution, obstacles_dynamic_pos_list, start_time, limited_time):
        ...
        if IS_DEBUG:
            print("[INFO] A* start to search " + str(agent_name))
            start_time_crr = time.time()
        ...
        while focal_list:
            ...
            if self.is_at_goal(current, agent_name):
                if IS_DEBUG:
                    end_time_crr = time.time()
                    print("[INFO] " + str(agent_name) + " computing ends. Use time: " + str(end_time_crr - start_time_crr) + " .")
