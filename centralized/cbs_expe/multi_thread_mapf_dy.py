...
import time
import eventlet
from cbs_expe.cbs_mine_dy import *
import threading
    ...
    def run(self):
        ...
        while True:
            ...
            if pq.empty():
                ...
                # min_anytime_limitation = DEFAULT_TIME_LIMITATION

                anytime_limitation_ls = []
                for i in range(self.num_combine):
                    ...
                    anytime_limitation_ls.append(anytime_limitation)
                    if anytime_limitation < min_anytime_limitation:
                        min_anytime_limitation = anytime_limitation
                

                # anytime_limitation_ls = sorted(anytime_limitation_ls)
                # for anytime_limitation in anytime_limitation_ls:
                #     if anytime_limitation == 0:
                #         continue
                #     min_anytime_limitation = anytime_limitation
                #     break
                ...
                # env = Environment(self.dimension, agents_cp, self.obstacles + conflicts)
                # Searching
...
class Utils(object):
    ...
    def anytime_func(self, cost):
        # return cost // 2
        # return 1 if (cost - 1) == 0 else cost - 1
        return cost - 1
