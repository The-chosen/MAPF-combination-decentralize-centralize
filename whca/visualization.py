...
class Graph(object):
    ...
    def show_obstacles(self, obstacles):
        ...
        for point in obstacles:
            cv2.circle(self.background, (int(point[0]* 10), int(point[1]* 10)), 5, (255, 0, 0), -1, 0)
                ...
                for i in range(0, np.array(trajectories).shape[1]-1, 1):
                    for rob in range(np.asarray(trajectories).shape[0]):
                        cv2.line(self.background,
                                 (int(trajectories[rob][i][0] * 10), int(trajectories[rob][i][1] * 10)),
                                 (int(trajectories[rob][i+1][0] * 10), int(trajectories[rob][i+1][1] * 10)),
                                 (0, 0, 255), 5, -1, 0)
        ...
        for edge in paths:
            ...
            for point in edge:
                if last_point is not None:
                    cv2.line(self.background, last_point, (int(point[0]* 10), int(point[1]* 10)),
                             (0, 0, 255), 5, -1, 0)
                last_point = (int(point[0]), int(point[1]))

    def show_agent(self, points, color=(0, 255, 255)):
        for point in points:
            cv2.circle(self.background, (int(point[0]* 10), int(point[1]* 10)), 5, (0, 255, 255), -1, 0)
