...
def create_trigonometry_controller(linear_vel_gain=0.5, angular_vel_gain=0.5, velocity_magnitude_limit=0.15,angular_velocity_limit=np.pi, position_error=0.05, rotation_error=0.05):

    #Check user input types
    assert isinstance(linear_vel_gain, (int, float)), "In the function create_trigonometry_controller, the x linear velocity gain (linear_vel_gain) must be an integer or float. Recieved type %r." % type(linear_vel_gain).__name__
    assert isinstance(angular_vel_gain, (int, float)), "In the function create_trigonometry_controller, the y linear velocity gain (angular_vel_gain) must be an integer or float. Recieved type %r." % type(angular_vel_gain).__name__
    assert isinstance(velocity_magnitude_limit, (int, float)), "In the function create_trigonometry_controller, the velocity magnitude limit (angular_vel_gain) must be an integer or float. Recieved type %r." % type(angular_vel_gain).__name__
    
    #Check user input ranges/sizes
    assert linear_vel_gain > 0, "In the function create_trigonometry_controller, the x linear velocity gain (linear_vel_gain) must be positive. Recieved %r." % linear_vel_gain
    assert angular_vel_gain > 0, "In the function create_trigonometry_controller, the y linear velocity gain (angular_vel_gain) must be positive. Recieved %r." % angular_vel_gain
    assert velocity_magnitude_limit >= 0, "In the function create_trigonometry_controller, the velocity magnitude limit (velocity_magnitude_limit) must not be negative. Recieved %r." % velocity_magnitude_limit

    def trigonometry_controller(states,poses):
        N=states.shape[1]
        dxu = np.zeros((2,N))
        ...
        for i in range(N):
            ...
            if norm > position_error:
                if(norm > velocity_magnitude_limit):
                    dxi = velocity_magnitude_limit*dxi/norm
                V = linear_vel_gain*np.linalg.norm(dxi) 
                dxu[0,i] = V*np.cos(wrapped)
                dxu[1,i] = V/l*np.sin(wrapped)
            else:
                dxu[:,[i]] = np.zeros((2,1))

        return dxu

    return trigonometry_controller

def create_tracking_controller(linear_vel_gain=0.5, angular_vel_gain=0.5, velocity_magnitude_limit=0.15,angular_velocity_limit=np.pi, position_error=0.05, rotation_error=0.05):

    #Check user input types
    assert isinstance(linear_vel_gain, (int, float)), "In the function create_tracking_controller, the x linear velocity gain (linear_vel_gain) must be an integer or float. Recieved type %r." % type(linear_vel_gain).__name__
    assert isinstance(angular_vel_gain, (int, float)), "In the function create_tracking_controller, the y linear velocity gain (angular_vel_gain) must be an integer or float. Recieved type %r." % type(angular_vel_gain).__name__
    assert isinstance(velocity_magnitude_limit, (int, float)), "In the function create_tracking_controller, the velocity magnitude limit (angular_vel_gain) must be an integer or float. Recieved type %r." % type(angular_vel_gain).__name__
    
    #Check user input ranges/sizes
    assert linear_vel_gain > 0, "In the function create_tracking_controller, the x linear velocity gain (linear_vel_gain) must be positive. Recieved %r." % linear_vel_gain
    assert angular_vel_gain > 0, "In the function create_tracking_controller, the y linear velocity gain (angular_vel_gain) must be positive. Recieved %r." % angular_vel_gain
    assert velocity_magnitude_limit >= 0, "In the function create_tracking_controller, the velocity magnitude limit (velocity_magnitude_limit) must not be negative. Recieved %r." % velocity_magnitude_limit

    def tracking_controller(states,poses):
        N=states.shape[1]
        dxu = np.zeros((2,N))
        k1 = 0.2
        k2 = 0.5
        ...
        for i in range(N):
            ...
            if norm > position_error:
                if(norm > velocity_magnitude_limit):
                    dxi = velocity_magnitude_limit*dxi/norm
                vr = linear_vel_gain*norm
                wr = angular_vel_gain*wrapped
                u1 = -k1*dxi[0]
                u2 = k2*vr*np.sinc(wrapped)*dxi[1]-k3*wrapped
                # print("vr ",vr," wr ",wr," u1 ",u1," u2 ",u2,)
                dxu[0,i] = vr*np.cos(wrapped) - u1
                dxu[1,i] = wr-u2
